...
In this lab, you create a simple console application in which you will use a Mutex to ensure there is only one instance of the application running at any point.
...
6. Inside the try section of the try/catch block, call the Mutex.OpenExisting method, using the constant string defined in step 4 as the name of the Mutex.
...
namespace SingleInstance {
	class SingleInstance {
		...
		static void Main (string[] args) {
			Mutex oneMutex = null;
			const string MutexName = "RUNMEONCEONLY";

			try { oneMutex = Mutex.OpenExisting(MutexName); }	//Try and open the Mutex
			catch (WaitHandleCannotBeOpenedException) { /*Cannot open the mutex because it doesn't exist*/ }

			if (oneMutex == null) { oneMutex = new Mutex(true, MutexName); }	//Create it if doesn't exist
			else {oneMutex.Close(); }	//Close the mutex and exit app because we can only have one instance
